  ...
  template: `
    <div>
      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-ink">Company Groups</h1>
      ...
      @if (loading()) {
        ...
        <div class="app-panel">
          <div class="app-panel-header app-panel-header--amber">
            ...
            <div>
              <h2 class="text-base font-semibold text-gray-900 dark:text-ink">Group Allocation</h2>
              <p class="mt-1 text-sm text-gray-600 dark:text-ink-dim">Define customer groups that share credit capacity and commercial exposure.</p>
            </div>
          </div>
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              ...
              @for (group of groups(); track group.id) {
                    ...
                    @if (group.companyNames.length) {
                      <div class="flex flex-wrap gap-1">
                        @for (name of group.companyNames; track name) {
                          <span class="inline-flex items-center rounded-full bg-amber-50 dark:bg-amber-500/15 px-2 py-0.5 text-xs font-medium text-amber-700 dark:text-amber-400">
                            {{ name }}
                          </span>
                        }
                      </div>
                    } @else {
                      <span class="text-gray-400 dark:text-muted">No companies</span>
                    }
                  </td>
                  <td class="px-4 py-3">
                    <div class="flex items-center gap-1">
                      <button (click)="openEditModal(group)" class="rounded-md p-1 text-gray-400 dark:text-muted hover:text-brand-600 transition-colors" title="Edit">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                          <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" />
                          <path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" />
                        </svg>
                      </button>
                      <button (click)="confirmDelete(group)" class="rounded-md p-1 text-gray-400 dark:text-muted hover:text-red-500 transition-colors" title="Delete">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                ...
                <tr>
                  <td colspan="3" class="px-4 py-8 text-center text-gray-400 dark:text-muted">
      ...
      @if (showModal()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div class="rounded-xl bg-white dark:bg-surface p-6 shadow-xl w-full max-w-lg mx-4" (click)="$event.stopPropagation()">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-ink">{{ editingId() ? 'Edit' : 'Create' }} Company Group</h3>
            ...
            <div class="mt-4 space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim">Group Name *</label>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim mb-2">Company Search</label>
                <input type="text" [ngModel]="searchQuery()" (ngModelChange)="onSearchChange($event)"
                  class="app-input w-full"
                  placeholder="Search companies to add…" />

                @if (searchResults().length) {
                  <div class="mt-1 max-h-36 overflow-y-auto rounded-lg border border-gray-200 dark:border-line divide-y divide-gray-100 dark:divide-line">
                    @for (co of searchResults(); track co.id) {
                      <button (click)="addCompany(co)"
                        class="w-full flex items-center gap-2 px-3 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-surface-tint">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 dark:text-muted" viewBox="0 0 20 20" fill="currentColor">
                          <path d="M10.75 4.75a.75.75 0 0 0-1.5 0v4.5h-4.5a.75.75 0 0 0 0 1.5h4.5v4.5a.75.75 0 0 0 1.5 0v-4.5h4.5a.75.75 0 0 0 0-1.5h-4.5v-4.5Z" />
                        </svg>
                        <span class="text-gray-900 dark:text-ink">{{ co.name }}</span>
                        @if (co.country) {
              ...
              @if (selectedCompanies().length) {
                <div>
                  <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim mb-2">Selected Companies</label>
                  <div class="flex flex-wrap gap-1.5">
                    @for (co of selectedCompanies(); track co.id) {
                      <span class="inline-flex items-center gap-1 rounded-full bg-amber-50 dark:bg-amber-500/15 px-2.5 py-1 text-xs font-medium text-amber-700 dark:text-amber-400">
                        {{ co.name }}
      ...
      @if (deleteTarget()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" (click)="deleteTarget.set(null)">
          <div class="rounded-xl bg-white dark:bg-surface p-6 shadow-xl max-w-sm mx-4" (click)="$event.stopPropagation()">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-ink">Delete group?</h3>

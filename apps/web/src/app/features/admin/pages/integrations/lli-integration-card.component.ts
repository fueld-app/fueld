    ...
    <div class="app-panel">
      <div class="app-panel-header app-panel-header--blue">
        ...
        <div class="flex-1 min-w-0">
          <h3 class="text-base font-semibold text-gray-900 dark:text-ink">Lloyd's List Intelligence / Seasearcher</h3>
          <p class="text-sm text-gray-500 dark:text-muted">Vessel tracking &amp; port data from Lloyd's List Intelligence.</p>
        </div>
        <div>
      ...
      @if (status()?.configured) {
        <div class="border-b border-gray-100 dark:border-line bg-gray-50/50 px-6 py-3 dark:bg-surface-2">
          <div class="flex items-center gap-6 text-sm">
            <div>
              <span class="text-gray-500 dark:text-muted">Username:</span>
              <span class="ml-1.5 font-medium text-gray-900 dark:text-ink">{{ status()!.username }}</span>
            </div>
            @if (status()!.updatedAt) {
              <div>
                <span class="text-gray-500 dark:text-muted">Last updated:</span>
      ...
      <div class="px-6 py-5">
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim">Username / Email</label>
        ...
        <div class="mt-5 flex items-center gap-3">
          <button (click)="saveLLICredentials()" [disabled]="saving()"
            class="inline-flex items-center gap-2 rounded-lg bg-brand-700 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-brand-800 disabled:opacity-50 transition-colors">
            @if (saving()) {
          ...
          <span class="text-xs text-gray-400 dark:text-muted">Credentials will be verified against the LLI API before saving.</span>
        </div>
      </div>
    </div>
  `,

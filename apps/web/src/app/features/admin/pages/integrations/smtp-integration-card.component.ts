    ...
    <div class="app-panel">
      <div class="app-panel-header app-panel-header--indigo">
        ...
        <div class="flex-1 min-w-0">
          <h3 class="text-base font-semibold text-gray-900 dark:text-ink">SMTP (Invite Emails)</h3>
          <p class="text-sm text-gray-500 dark:text-muted">Send automatic user invites via SMTP.</p>
        </div>
        <div>
      ...
      <div class="px-6 py-5">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim">SMTP Host</label>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim">Port</label>
            <input type="number" [ngModel]="smtpPort()" (ngModelChange)="smtpPort.set($event)"
              class="mt-1 w-full rounded-lg border border-gray-300 dark:border-line-strong px-3 py-2 text-sm focus:border-brand-600 focus:ring-1 focus:ring-brand-600 outline-none"
              placeholder="587" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim">Username</label>
            <input type="text" [ngModel]="smtpUser()" (ngModelChange)="smtpUser.set($event)"
              class="mt-1 w-full rounded-lg border border-gray-300 dark:border-line-strong px-3 py-2 text-sm focus:border-brand-600 focus:ring-1 focus:ring-brand-600 outline-none"
              placeholder="smtp-user" autocomplete="username" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim">Password</label>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim">From Address</label>
          ...
          <div class="flex items-center gap-2 pt-6">
            ...
            <label for="smtp-secure" class="text-sm text-gray-700 dark:text-ink-dim">Use TLS/SSL</label>
          </div>
        </div>
        ...
        <div class="mt-5 flex items-center gap-3">
          ...
          <span class="text-xs text-gray-400 dark:text-muted">Credentials are stored encrypted.</span>
        </div>

        <div class="mt-6 border-t border-gray-100 dark:border-line pt-5">
          @if (smtpTestSuccess()) {
            <div class="mb-4 flex items-center gap-2 rounded-lg bg-green-50 dark:bg-green-500/15 border border-green-200 dark:border-green-500/30 p-3 text-sm text-green-700 dark:text-green-400">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
          ...
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim">Test email address</label>
          ...
          <div class="mt-4 flex items-center gap-3">
            <button (click)="sendSmtpTest()" [disabled]="smtpTestSending()"
              class="inline-flex items-center gap-2 rounded-lg border border-indigo-300 bg-white dark:bg-surface px-4 py-2 text-sm font-medium text-indigo-700 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/15 disabled:opacity-50 transition-colors">
              @if (smtpTestSending()) {
                <svg class="h-4 w-4 animate-spin" viewBox="0 0 24 24" fill="none">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>

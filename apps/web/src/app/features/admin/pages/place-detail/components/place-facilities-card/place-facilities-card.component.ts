  ...
  template: `
    <div class="app-panel h-[420px] flex flex-col">
      <div class="app-panel-header app-panel-header--blue px-5 py-3">
        <h2 class="text-sm font-semibold text-gray-700 dark:text-ink-dim">Port Facilities</h2>
      </div>
      ...
      <div class="flex-1 overflow-y-auto">
        @if (store.facilitiesLoading()) {
          ...
          <div class="px-5 py-6 text-center text-sm text-gray-400 dark:text-muted">No facilities registered</div>
        } @else {
          <div class="divide-y divide-gray-50">
            @for (facility of store.facilities(); track facility.id) {
              <div class="px-5 py-3">
                <div class="flex items-center justify-between">
                  <span class="text-sm font-medium text-gray-800 dark:text-ink">{{ facility.label || facility.name }}</span>
                ...
                @if (facility.text || facility.description) {
                  <p class="mt-1 text-xs text-gray-500 dark:text-muted">{{ facility.text || facility.description }}</p>
                }
                @if (facility.maxSize || facility.maxDwt || facility.capacity) {
                  <div class="mt-1.5 flex flex-wrap gap-2 text-[10px] text-gray-400 dark:text-muted">

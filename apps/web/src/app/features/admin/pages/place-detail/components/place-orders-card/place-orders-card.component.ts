  ...
  template: `
    <div class="app-panel h-[420px] flex flex-col">
      <div class="app-panel-header app-panel-header--blue justify-between px-5 py-3">
        <h2 class="text-sm font-semibold text-gray-700 dark:text-ink-dim">Orders
      ...
      <div class="flex-1 overflow-y-auto">
        @if (store.ordersLoading()) {
          <div class="px-5 py-6 text-center text-sm text-gray-400 dark:text-muted">Loading orders…</div>
        } @else if (!store.placeOrders().length) {
          <div class="px-5 py-6 text-center text-sm text-gray-400 dark:text-muted">No orders for this place</div>
        } @else {
          <div class="divide-y divide-gray-50">
            @for (order of store.placeOrders(); track order.id) {
              ...
              >
                ...
                <div class="mt-1 flex items-center gap-2 text-[10px] text-gray-400 dark:text-muted">

  ...
  template: `
    <div class="app-panel h-[420px] flex flex-col">
      ...
      @if (store.showAddSupplier()) {
        <div class="border-b border-gray-100 px-5 py-3 space-y-3">
          <div class="relative">
            <input
              type="text"
          ...
          @if (store.supplierCompanyResults().length) {
            <div class="max-h-48 overflow-y-auto rounded-md border border-gray-100 shadow-sm">
              @for (company of store.supplierCompanyResults(); track company.key) {
                ...
                >
                  ...
                  @if (company.country) {
                    <span class="ml-1 text-xs text-gray-400">· {{ company.country }}</span>
                  }
                </button>
              }
            </div>
          ...
          @if (store.selectedSupplierCompany(); as selected) {
            <div class="text-sm text-gray-700">Selected: <span class="font-medium">{{ selected.name }}</span></div>
            ...
            <div>
              <label class="block text-xs font-medium text-gray-600 mb-1">Products</label>

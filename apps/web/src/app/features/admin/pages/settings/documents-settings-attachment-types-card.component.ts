    ...
    <div class="app-panel">
      <div class="app-panel-header app-panel-header--indigo">
        ...
        <div class="flex-1 min-w-0">
          <h3 class="text-sm font-semibold text-gray-900 dark:text-ink">Attachment Types</h3>
          <p class="text-xs text-gray-500 dark:text-muted">Configure which attachment types can be selected when uploading order/inquiry attachments.</p>
        </div>
      </div>
        ...
        @for (type of types(); track $index; let i = $index) {
          <div class="flex items-center gap-2">
            <div class="flex flex-col gap-0.5 shrink-0">
              ...
              <button (click)="moveDown.emit(i)" [disabled]="i === types().length - 1" class="text-gray-400 dark:text-muted hover:text-gray-600 disabled:opacity-30 transition-colors" title="Move down">

      ...
      @if (loading()) {
        ...
        <div class="grid grid-cols-1 gap-6 min-[900px]:grid-cols-2 min-[1600px]:grid-cols-3 min-[2000px]:grid-cols-4">
          ...
          <app-documents-settings-attachment-types-card
            ...
            (moveUp)="moveAttachmentTypeUp($event)"
            (moveDown)="moveAttachmentTypeDown($event)"
            (typeChange)="updateAttachmentType($event.index, $event.value)"
            (remove)="removeAttachmentType($event)"
            (add)="addAttachmentType()"
            (save)="saveAttachmentTypes()"
          />

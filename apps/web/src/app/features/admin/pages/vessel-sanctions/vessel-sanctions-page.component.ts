...
@Component({
  ...
  template: `
    <div class="mx-auto max-w-5xl space-y-6 p-6">
      <div>
        <h1 class="text-lg font-semibold text-gray-900">Vessel Sanctions</h1>
        <p class="mt-1 text-sm text-gray-500">
          ...
          <a href="https://tankertrackers.com/report/sanctioned" target="_blank" rel="noopener noreferrer"
            class="text-brand-600 underline hover:text-brand-700">TankerTrackers sanctioned vessel list</a>.
        </p>
      </div>
      ...
      @if (loading()) {
        ...
        <div class="grid gap-6 lg:grid-cols-2">
          ...
          <div class="app-panel">
            <div class="app-panel-header app-panel-header--amber">
              ...
              <div class="flex-1 min-w-0">
                <h3 class="text-sm font-semibold text-gray-900">Sanction Check Settings</h3>
                <p class="text-xs text-gray-600">Configure automated vessel sanction screening.</p>
              </div>
            </div>
            ...
            <div class="app-panel-body app-panel-stack">
              ...
              <div class="flex items-start gap-3">
                ...
                <div>
                  <p class="text-sm font-medium text-gray-700">Enable Vessel Sanction Checks</p>
                  <p class="text-xs text-gray-500">Automatically check all vessels against the TankerTrackers sanctioned list.</p>
                </div>
              </div>
              ...
              @if (enabled()) {
                ...
                <div>
                  ...
                  />
                  <p class="mt-1 text-xs text-gray-500">How often to re-check all vessels. Default: 24 hours.</p>
                </div>
                ...
                <div class="space-y-2">
                  ...
                  <div class="flex items-start gap-3">
                    <button (click)="notifyPush.set(!notifyPush())"
                      class="relative mt-0.5 inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors"
                      [class]="notifyPush() ? 'bg-brand-600' : 'bg-gray-300'">
                      <span class="inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform"
                        [class]="notifyPush() ? 'translate-x-4' : 'translate-x-0.5'"></span>
                    </button>
                    <div>
                      <p class="text-sm text-gray-700">Push Notifications</p>
                      <p class="text-xs text-gray-500">Notify admins and credit managers via push when sanctions are detected.</p>
                    </div>
                  </div>
                  ...
                  <div class="flex items-start gap-3">
                    ...
                    <div>
                      <p class="text-sm text-gray-700">Email Notifications</p>
                    </div>
                  </div>
                  ...
                  <div class="flex items-start gap-3">
                    ...
                    <div>
                      <p class="text-sm text-gray-700">WhatsApp Notifications</p>
                    </div>
                  </div>
                </div>
              }
              ...
              <div class="pt-2 border-t border-gray-100">
                ...
                @if (saveSuccess()) {
                  <span class="ml-3 text-sm text-green-600">Settings saved.</span>
                }
                @if (saveError()) {
          ...
          <div class="app-panel">
            <div class="app-panel-header">
              <div class="flex-1 min-w-0">
                <h3 class="text-sm font-semibold text-gray-900">Manual Check</h3>
                <p class="text-xs text-gray-600">Trigger an immediate sanction check against all vessels in the database.</p>
              </div>
            </div>
            <div class="app-panel-body">
              ...
              @if (checkResult()) {
                <div class="mt-4 rounded-lg border p-3 text-sm space-y-1"
                  [class]="checkResult()!.sanctioned > 0 ? 'border-red-200 bg-red-50' : 'border-green-200 bg-green-50'">
                  <p><strong>Vessels checked:</strong> {{ checkResult()!.checked }}</p>
                  <p><strong>Sanctioned:</strong>
                    <span [class]="checkResult()!.sanctioned > 0 ? 'text-red-700 font-semibold' : 'text-green-700'">
                      {{ checkResult()!.sanctioned }}
                    </span>
                  </p>
                  @if (checkResult()!.errors > 0) {
                    <p><strong>Errors:</strong> <span class="text-amber-700">{{ checkResult()!.errors }}</span></p>
                  }
                </div>
              }

              @if (checkError()) {
                <div class="mt-3 text-sm text-red-600">{{ checkError() }}</div>
              }
            </div>
          </div>
        </div>
        ...
        <div class="app-panel">
          <div class="app-panel-header">
            <div class="flex-1 min-w-0">
              <h3 class="text-sm font-semibold text-gray-900">Check History</h3>
              <p class="text-xs text-gray-600">Recent vessel sanction check results (sanctioned vessels only shown by default).</p>
            </div>
          ...
          <div class="app-panel-body p-0">
            @if (historyLoading()) {
              <div class="p-4 text-sm text-gray-400">Loading history…</div>
            } @else if (history().length === 0) {
              <div class="p-4 text-sm text-gray-400">No sanction checks recorded yet.</div>
            } @else {
              <div class="overflow-x-auto">
                <table class="min-w-full text-xs">
                  <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/60">
                      <th class="px-4 py-2.5 text-left font-medium text-gray-500">Vessel</th>
                      <th class="px-4 py-2.5 text-left font-medium text-gray-500">IMO</th>
                      <th class="px-4 py-2.5 text-left font-medium text-gray-500">Status</th>
                      <th class="px-4 py-2.5 text-left font-medium text-gray-500">Matched On</th>
                      <th class="px-4 py-2.5 text-left font-medium text-gray-500">Source</th>
                      <th class="px-4 py-2.5 text-left font-medium text-gray-500">Checked At</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-gray-50">
                    @for (c of history(); track c.id) {
                      <tr class="hover:bg-gray-50/50">
                        <td class="px-4 py-2 font-medium text-gray-900">{{ c.vesselName }}</td>
                        <td class="px-4 py-2 text-gray-500 font-mono">{{ c.vesselImo || '—' }}</td>
                        <td class="px-4 py-2">
                          @if (c.status === 'SANCTIONED') {
                            ...
                            <span class="inline-flex rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700">Clear</span>
                          } @else {
                            <span class="inline-flex rounded-full bg-yellow-100 px-2 py-0.5 text-xs font-medium text-yellow-700">Error</span>
                          }
                        </td>
                        <td class="px-4 py-2 text-gray-500">{{ c.matchedOn || '—' }}</td>
                        <td class="px-4 py-2 text-gray-500">{{ c.source }}</td>
                        <td class="px-4 py-2 text-gray-500">{{ c.checkedAt | date:'medium' }}</td>
                      </tr>
                    }
                  </tbody>
                </table>
                ...
                <div class="flex items-center justify-between border-t border-gray-100 px-4 py-2 text-xs text-gray-500">
                  ...
                  <div class="flex gap-2">
                    <button (click)="historyPage.set(historyPage() - 1); loadHistory()"
                      ...
                      class="rounded px-2 py-1 hover:bg-gray-100 disabled:opacity-30">Previous</button>
                    <span>Page {{ historyPage() }}</span>
                    <button (click)="historyPage.set(historyPage() + 1); loadHistory()"
                      [disabled]="historyPage() * historyLimit >= historyTotal()"
                      class="rounded px-2 py-1 hover:bg-gray-100 disabled:opacity-30">Next</button>
                  </div>
                </div>
              }
            }

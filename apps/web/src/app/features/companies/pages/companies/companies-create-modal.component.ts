    ...
    @if (open()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div class="rounded-xl bg-white dark:bg-surface p-6 shadow-xl w-full max-w-lg mx-4" (click)="$event.stopPropagation()">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-ink">Create Company</h3>
          <p class="mt-1 text-sm text-gray-500 dark:text-muted">Add a company manually that isn't in Seasearcher.</p>
          ...
          <div class="mt-4 space-y-4">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="col-span-2">
                <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim mb-1">Type(s) *</label>
                <div class="flex flex-wrap gap-3">
                  @for (opt of typeOptions(); track opt.value) {
                    <label class="inline-flex items-center gap-1.5 cursor-pointer">
                      ...
                      <span class="text-sm text-gray-700 dark:text-ink-dim">{{ opt.label }}</span>
                    </label>
                  }
                </div>
              </div>

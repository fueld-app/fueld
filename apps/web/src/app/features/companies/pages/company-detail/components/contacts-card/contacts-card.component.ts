...
@Component({
  ...
  imports: [FormsModule],
  template: `
    <div class="rounded-xl border border-gray-200 dark:border-line bg-white dark:bg-surface shadow-sm min-[900px]:order-5">
      <div class="border-b border-gray-100 dark:border-line px-5 py-3 flex items-center justify-between">
        <div class="flex items-center gap-2">
          <h2 class="text-sm font-semibold text-gray-700 dark:text-ink-dim">Contacts</h2>
          @if (contacts().length) {
        ...
        <button
          (click)="openAdd()"
          class="inline-flex items-center gap-1 rounded-md bg-brand-50 dark:bg-brand-700/15 px-2 py-1 text-[11px] font-medium text-brand-700 dark:text-brand-400 hover:bg-brand-100 dark:hover:bg-brand-500/20 transition-colors"
        >
      ...
      @if (contactsLoading()) {
            ...
            <div class="px-5 py-3 flex items-start justify-between group">
              <div class="flex items-start gap-3 min-w-0">
                <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-semibold"
                  [class]="c.source === 'seasearcher' ? 'bg-blue-50 dark:bg-blue-500/15 text-blue-700 dark:text-blue-400' : 'bg-gray-100 dark:bg-surface-3 text-gray-600 dark:text-ink-dim'">
                  {{ c.name.charAt(0) }}
                </div>
                <div class="min-w-0">
                  <div class="flex items-center gap-2">
                    ...
                    @if (c.role) {
                      <span class="inline-flex items-center rounded-full bg-gray-100 dark:bg-surface-3 px-2 py-0.5 text-[10px] font-medium text-gray-600 dark:text-ink-dim">{{ c.role }}</span>
                    }
                    @if (c.source === 'seasearcher') {
                      <span class="inline-flex items-center rounded-full bg-blue-50 dark:bg-blue-500/15 px-1.5 py-0.5 text-[10px] font-medium text-blue-600 dark:text-blue-400">LLI</span>
                    }
                  </div>
                  <div class="mt-0.5 flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-gray-500 dark:text-muted">
                    @if (c.phone) {
                    ...
                    @if (c.email) {
                      <a [href]="'mailto:' + c.email" class="inline-flex items-center gap-1 text-brand-600 dark:text-brand-400 hover:text-brand-800">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
                  ...
                  @if (c.notes) {
                    <p class="mt-1 text-xs text-gray-400 dark:text-muted italic">{{ c.notes }}</p>
                  }
                </div>
              </div>
              <div class="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0 ml-2">
                <button (click)="openEdit(c)" class="rounded-md p-1 text-gray-400 dark:text-muted hover:text-brand-600 transition-colors" title="Edit">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor">
                    <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" />
                    <path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" />
                  </svg>
                </button>
                <button (click)="confirmDelete(c)" class="rounded-md p-1 text-gray-400 dark:text-muted hover:text-red-500 transition-colors" title="Delete">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div class="px-5 py-6 text-center text-sm text-gray-400 dark:text-muted">No contacts yet. Click "Add Contact" to add one.</div>
      }
    </div>
    ...
    @if (showModal()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div class="rounded-xl bg-white dark:bg-surface p-6 shadow-xl w-full max-w-md mx-4" (click)="$event.stopPropagation()">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-ink">{{ editingId() ? 'Edit' : 'Add' }} Contact</h3>
          @if (formError()) {
          ...
          <div class="mt-4 space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim">Phone</label>
                <input type="text" [ngModel]="form().phone" (ngModelChange)="updateField('phone', $event)"
                  class="mt-1 w-full rounded-lg border border-gray-300 dark:border-line-strong px-3 py-2 text-sm focus:border-brand-600 focus:ring-1 focus:ring-brand-600 outline-none"
                  placeholder="+1 905 467 7357" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim">Fax</label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-ink-dim">Email</label>
    ...
    @if (deleteTarget()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" (click)="deleteTarget.set(null)">
        <div class="rounded-xl bg-white dark:bg-surface p-6 shadow-xl max-w-sm mx-4" (click)="$event.stopPropagation()">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-ink">Delete contact?</h3>
          <p class="mt-2 text-sm text-gray-500 dark:text-muted">Are you sure you want to delete <strong>{{ deleteTarget()!.name }}</strong>?</p>

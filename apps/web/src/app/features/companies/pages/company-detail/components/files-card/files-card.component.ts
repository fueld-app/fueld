    ...
    <div class="rounded-xl border border-gray-200 dark:border-line bg-white dark:bg-surface shadow-sm min-[900px]:order-[10]">
      <div class="border-b border-gray-100 dark:border-line px-5 py-3 flex items-center justify-between">
        <div>
          ...
          <p class="mt-0.5 text-xs text-gray-400 dark:text-muted">Upload PDFs and spreadsheets with operational notes or supplier documentation.</p>
        </div>
      </div>
      <div class="px-5 py-4 space-y-4">
          ...
          <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div class="min-w-0">
              <input
                ...
                class="w-full text-sm text-gray-600 dark:text-ink-dim file:mr-4 file:rounded-lg file:border-0 file:bg-white file:px-3 file:py-2 file:text-sm file:font-medium file:text-gray-700 hover:file:bg-gray-100"
              />
              <p class="mt-2 text-xs text-gray-400 dark:text-muted">Accepted: PDF, XLS, XLSX, CSV or image files. Max 10 MB.</p>
              @if (selectedFile) { <p class="mt-1 text-xs text-gray-500 dark:text-muted">Ready to upload: {{ selectedFile!.name }}</p> }
            </div>
            <button type="button"
              (click)="upload()"
              [disabled]="uploading() || !selectedFile"
              class="inline-flex items-center justify-center rounded-lg bg-brand-700 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-brand-800 disabled:opacity-50">
        ...
        @if (loading()) {
          ...
          <div class="rounded-lg border border-gray-100 dark:border-line bg-white dark:bg-surface px-4 py-6 text-center text-sm text-gray-400 dark:text-muted">No files uploaded yet</div>
        } @else {
            ...
            @for (a of attachments(); track a.id) {
              <div class="flex items-start justify-between gap-3 px-4 py-3 text-sm hover:bg-gray-50/70 transition-colors dark:hover:bg-surface-tint">
                <div class="min-w-0 flex-1">
                  ...
                  <div class="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-400 dark:text-muted">
                ...
                <button type="button" (click)="confirmDelete(a)"
                  class="rounded-md p-1 text-gray-400 dark:text-muted hover:text-red-500 transition-colors" title="Delete file">
    ...
    @if (deleteTarget()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" (click)="deleteTarget.set(null)">
        <div class="rounded-xl bg-white dark:bg-surface p-6 shadow-xl max-w-sm mx-4" (click)="$event.stopPropagation()">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-ink">Delete file?</h3>
          <p class="mt-2 text-sm text-gray-500 dark:text-muted">Remove <strong>{{ deleteTarget()!.fileName }}</strong> from this company?</p>

  ...
  template: `
    <div class="rounded-xl border border-gray-200 dark:border-line bg-white dark:bg-surface shadow-sm min-[900px]:order-8 flex flex-col overflow-hidden">
      <div class="border-b border-gray-100 dark:border-line px-5 py-3 flex items-center justify-between">
        <h2 class="text-sm font-semibold text-gray-700 dark:text-ink-dim">Registration & Ownership</h2>
        <div class="flex gap-1">
          <button type="button"
            class="rounded-md px-2.5 py-1 text-[11px] font-medium transition-colors"
            [class]="tab() === 'ownership' ? 'bg-brand-50 dark:bg-brand-700/15 text-brand-700 dark:text-brand-400' : 'text-gray-400 dark:text-muted hover:text-gray-600'"
            (click)="tab.set('ownership')">Ownership Structure</button>
          <button type="button"
            class="rounded-md px-2.5 py-1 text-[11px] font-medium transition-colors"
            [class]="tab() === 'registration' ? 'bg-brand-50 dark:bg-brand-700/15 text-brand-700 dark:text-brand-400' : 'text-gray-400 dark:text-muted hover:text-gray-600'"
      ...
      <div class="flex-1 min-h-0 overflow-y-auto px-5 py-4 space-y-3 text-sm">
        @if (tab() === 'registration') {
          @if (enrichment()?.companyRegistration; as reg) {
            <div class="space-y-2">
              @if (reg.localName) {
                <div class="flex justify-between"><span class="text-gray-500 dark:text-muted">Local Name</span><span class="font-medium text-gray-900 dark:text-ink">{{ reg.localName }}</span></div>
              }
              @if (reg.registryName) {
                <div class="flex justify-between"><span class="text-gray-500 dark:text-muted">Registry</span><span class="font-medium text-gray-900 dark:text-ink">{{ reg.registryName }}</span></div>
              }
              @if (reg.incorporationDate) {
                <div class="flex justify-between"><span class="text-gray-500 dark:text-muted">Incorporated</span><span class="font-medium text-gray-900 dark:text-ink">{{ reg.incorporationDate | dateLabel }}</span></div>
              }
              @for (r of reg.registrationNumbers; track $index) {
                @if (r.value) {
                  <div class="flex justify-between">
                    <span class="text-gray-500 dark:text-muted">{{ r.typeDescription ?? 'Reg #' }}</span>
                    <span class="font-medium text-gray-900 dark:text-ink font-mono text-xs">{{ r.value }}</span>
                  </div>
                }
              }
            </div>
          } @else {
            <div class="text-xs text-gray-500 dark:text-muted text-center">Registration data unavailable</div>
          }
        } @else {
          @if (hierarchy(); as h) {
                ...
                <div class="space-y-1">
                  @for (node of flatNodes(); track $index) {
                    <div class="flex items-center gap-2 text-sm" [style.padding-left.px]="(node.level - 1) * 20">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-gray-300 dark:text-muted flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                        @if (node.level === 1) {
                      ...
                      @if (node.companyId === seasearcherId()) {
                        <span class="font-medium text-brand-600 dark:text-brand-400">{{ node.companyName }}</span>
                      } @else {
                        <button
                          (click)="navigateToCompany.emit(node.companyId)"
                          [disabled]="navigatingCompanyId() === node.companyId"
                          class="font-medium text-gray-900 dark:text-ink hover:text-brand-600 hover:underline text-left disabled:opacity-50">
                      ...
                      @if (node.isSanctioned) { <span class="text-xs text-red-600 dark:text-red-400">⚠️</span> }
                      @if (!node.active) { <span class="text-xs text-gray-400 dark:text-muted">(inactive)</span> }
                      <span class="text-xs text-gray-400 dark:text-muted ml-auto">{{ hierarchyRoles(node) }}</span>
                    </div>
                  }
                </div>
              } @else {
                <p class="text-sm text-gray-400 dark:text-muted text-center">No hierarchy data</p>
              }
            </div>
          } @else {
            <div class="text-xs text-gray-500 dark:text-muted text-center">Ownership data unavailable</div>
          }
        }
      </div>
    </div>

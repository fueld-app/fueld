...
@Component({
  ...
  template: `
    <div>
      ...
      <!-- Create / Edit modal -->
      @if (showModal()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" (click)="showModal.set(false)">
          <div class="rounded-xl bg-white p-6 shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" (click)="$event.stopPropagation()">
            <h3 class="text-lg font-semibold text-gray-900">{{ editingId() ? 'Edit' : 'Add' }} Customer Credit Line</h3>
            ...
            <div class="mt-4 space-y-4">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700">Customer(s) *</label>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  Our Companies
                  <span class="text-xs text-gray-400 ml-1">(which of our entities is giving this credit?)</span>
                </label>
              ...
              <div class="flex items-center gap-6">
                <label class="inline-flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" [ngModel]="form().fromDelivery" (ngModelChange)="updateForm('fromDelivery', $event)"
                    class="h-4 w-4 rounded border-gray-300 text-brand-600 focus:ring-brand-500" />
                  <span class="text-sm text-gray-700">From Delivery</span>
                </label>
                <label class="inline-flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" [ngModel]="form().qualified" (ngModelChange)="updateForm('qualified', $event)"
                    class="h-4 w-4 rounded border-gray-300 text-brand-600 focus:ring-brand-500" />
                  <span class="text-sm text-gray-700">Qualified</span>
                </label>
              </div>
...
export class CustomerCreditPageComponent implements OnInit {
  ...
  readonly form = signal<{
    ...
    qualified: boolean;
    notes: string;
  }>({ creditAmount: '', currency: 'USD', expires: '', periodDays: 30, fromDelivery: false, qualified: false, notes: '' });

    ...
    <div class="h-full flex flex-col">
      ...
      <div class="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-500">Track active orders by delivery status.</p>
        </div>
        <div class="flex flex-wrap items-end gap-3">
          <input
            ...
            class="w-64 rounded-lg border border-gray-300 px-4 py-2.5 text-sm shadow-sm
      ...
      @if (loading()) {
        ...
        <div class="flex flex-1 gap-4 overflow-x-auto pb-2">
          @for (column of columns(); track column.status) {
            <div class="flex w-80 min-w-[20rem] flex-col rounded-xl border border-gray-200 bg-gray-50/50">
              <!-- Column header -->
              ...
              <div class="flex-1 space-y-3 overflow-y-auto p-3">
                @for (order of column.orders; track order.id) {
                  <a
                    ...
                    class="block rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-all hover:shadow-md hover:border-brand-300"
                  >
                    <div class="flex items-start justify-between gap-2">
                      <div class="min-w-0">
                        <p class="text-xs font-mono text-gray-400">{{ order.orderNumber ?? '—' }}</p>
                        <p class="mt-0.5 truncate text-sm font-semibold text-gray-900">{{ order.clientName }}</p>
                      </div>
                  ...
                  <div class="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-200 py-8">

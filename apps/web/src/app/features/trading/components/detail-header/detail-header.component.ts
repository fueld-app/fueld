...
@Component({
  ...
  template: `
    <div class="mb-6">
      <!-- Breadcrumb -->
      <nav class="mb-4 flex items-center gap-1.5 text-sm text-gray-500 dark:text-muted">
        ...
        <span class="text-gray-900 dark:text-ink font-medium">{{ displayId() }}</span>
      </nav>
      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div class="flex items-center gap-3">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-ink">{{ title() }}</h1>
            <app-status-badge [status]="status()" />
          </div>
          <div class="mt-1 flex flex-wrap items-center gap-2 text-sm text-gray-500 dark:text-muted">
            @if (subtitle().trim()) {
              <p>{{ subtitle() }}</p>
            }
        ...
        <div class="flex items-center gap-3 sm:ml-auto">
          ...
          @if (showAutosave()) {
            <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-muted">
              @if (autoSaving()) {
                <svg class="h-4 w-4 animate-spin text-brand-600 dark:text-brand-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                </svg>
                <span>Saving...</span>
              } @else if (lastSaved()) {
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-green-500 dark:text-green-300" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">

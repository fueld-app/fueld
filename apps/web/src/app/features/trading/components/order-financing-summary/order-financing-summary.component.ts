    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
      <div class="flex flex-wrap items-stretch divide-x divide-gray-100">
        ...
        <div class="flex flex-1 flex-col items-center justify-center gap-0.5 px-3 py-3 min-w-[100px]">
          ...
          <span class="text-base font-bold tabular-nums" [class.text-green-600]="economics().totalGrossProfit > 0" [class.text-red-600]="economics().totalGrossProfit < 0">
            {{ economics().totalGrossProfit | number:'1.2-0' }}
          </span>
        </div>
        ...
        <div class="flex flex-1 flex-col items-center justify-center gap-0.5 px-3 py-3 min-w-[100px]">
          ...
          <span class="text-base font-bold tabular-nums text-amber-800">
            {{ economics().totalFinancingCost | number:'1.2-0' }}
          </span>
        </div>

        <!-- Net Profit -->
        <div class="flex flex-1 flex-col items-center justify-center gap-0.5 px-3 py-3 min-w-[100px]">
          <span class="text-[10px] font-medium uppercase tracking-wider text-gray-500">Net Profit</span>
          <span class="text-base font-bold tabular-nums" [class.text-green-600]="economics().totalNetProfit > 0" [class.text-red-600]="economics().totalNetProfit < 0">
            {{ economics().totalNetProfit | number:'1.2-0' }}
          </span>
        </div>
        ...
        <div class="flex flex-1 flex-col items-center justify-center gap-0.5 px-3 py-3 min-w-[80px]">
          ...
          <span class="text-base font-bold tabular-nums text-gray-900">
            {{ (economics().netMarginPct ?? 0) | number:'1.2-1' }}%
          </span>
        </div>
        ...
        <div class="hidden lg:flex flex-1 flex-col items-center justify-center gap-0.5 px-3 py-3 min-w-[80px]">
          ...
          <span class="text-base font-bold tabular-nums text-gray-900">
            {{ (economics().financingCostPerMt ?? 0) | number:'1.2-1' }}
          </span>
        </div>
        ...
        <div class="flex items-center gap-1.5 px-3 py-3 shrink-0">
          <span class="rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-semibold text-amber-700 whitespace-nowrap">
            {{ (financingRateAnnual() * 100) | number:'1.2-0' }}% · {{ financingDays() }}d
          </span>
        </div>
      </div>
    </div>

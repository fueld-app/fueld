    ...
    <div class="rounded-xl border border-gray-200 dark:border-line bg-white dark:bg-surface p-5 shadow-sm h-full max-h-[520px] flex flex-col">
      ...
      <div class="mt-4 flex-1 overflow-auto">
        @if (attachments().length === 0) {
          <p class="text-sm text-gray-400 dark:text-muted">No attachments yet.</p>
        } @else {
              ...
              <li class="flex items-center justify-between py-2 text-sm">
                <div class="flex items-center gap-2 min-w-0">
                  ...
                  <button
                    type="button"
                    (click)="open.emit(att)"
                    class="text-left text-brand-600 dark:text-brand-400 hover:underline truncate"
                  >
                    {{ att.fileName }}
                  </button>
                </div>
                <div class="flex items-center gap-2 shrink-0">
                  <span class="text-xs text-gray-400 dark:text-muted">{{ formatFileSize(att.fileSize) }}</span>
                  <button
                    type="button"

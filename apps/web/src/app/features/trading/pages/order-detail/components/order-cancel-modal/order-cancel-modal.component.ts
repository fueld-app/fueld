      ...
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold text-gray-900">Cancel {{ targetLabel() }}</h3>
            <button type="button" (click)="close()" class="text-gray-400 hover:text-gray-600">✕</button>
          </div>
          <p class="mt-3 text-sm text-gray-600">Select a reason for cancelling this {{ targetLabel() }}.</p>
          <div class="mt-4">
            ...
            <select [ngModel]="selectedReason()" (ngModelChange)="selectedReason.set($event)"
              class="fueld-select-no-chevron mt-1 w-full appearance-none rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-700 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 bg-white">

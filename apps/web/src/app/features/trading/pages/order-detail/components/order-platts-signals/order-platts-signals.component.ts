  ...
  template: `
    ...
    >
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 class="text-sm font-semibold uppercase tracking-wider text-gray-700 dark:text-ink-dim">Platts Signals</h3>
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            (click)="refresh.emit()"
            [disabled]="loading()"
            class="inline-flex items-center rounded-lg bg-brand-700 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-brand-800 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      @if (error(); as err) {
        <div class="mt-4 rounded-lg border border-rose-200 dark:border-rose-500/30 bg-rose-50 dark:bg-rose-500/15 px-3 py-2 text-sm text-rose-700 dark:text-rose-400">
          {{ err }}
        </div>
      } @else if (loading() && !items().length) {
        <div class="mt-4 text-sm text-gray-500 dark:text-muted">Loading Platts matches...</div>
      } @else if (!items().length) {
        <div class="mt-4 text-sm text-gray-500 dark:text-muted">No Platts suggestions available for the current items yet.</div>
      } @else {
          ...
          @for (item of items(); track item.key) {
            <div class="rounded-xl border border-gray-200 dark:border-line p-4">
              <div class="flex items-start justify-between gap-3">
                <div>
                  <div class="text-sm font-semibold text-gray-900 dark:text-ink">{{ item.productType }}</div>
                  @if (item.description) {
                    <div class="mt-1 text-xs text-gray-500 dark:text-muted">{{ item.description }}</div>
                  }
                </div>
                <div class="text-[11px] uppercase tracking-wide text-gray-400 dark:text-muted">{{ item.matches.length }} match{{ item.matches.length === 1 ? '' : 'es' }}</div>
              </div>

              @if (!item.matches.length) {
                <div class="mt-3 text-sm text-gray-500 dark:text-muted">No canonical Platts entries matched this line item.</div>
              } @else {
                  ...
                  @for (match of item.matches; track match.entryId) {
                    ...
                    >
                      <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500 dark:text-muted">
                        <span class="font-semibold text-gray-700 dark:text-ink-dim">{{ match.company || 'Market' }}</span>
                        @if (match.action) {
                      ...
                      <div class="mt-1 text-sm text-gray-800 dark:text-ink">{{ match.rawText }}</div>
                      <div class="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-gray-500 dark:text-muted">
                        @if (match.instrument) {

...
import type { ApiResponse, SupplierPaymentDto } from '@fueld/types';
import type { DropdownOption } from '@app/shared/components/searchable-dropdown/searchable-dropdown.component';
import { API_URL } from '@app/core/config/api';

/**
 * Supplier payment modal (two-sided order settlement).
 * Posts to /orders/:id/suppliers/:orderSupplierId/payments.
 */
@Component({
      ...
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div class="w-full max-w-lg rounded-xl bg-white dark:bg-surface p-6 shadow-xl">
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-ink">Record supplier payment</h3>
          ...
          <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <select [ngModel]="currency()" (ngModelChange)="currency.set($event)"
                class="fueld-select-no-chevron mt-1 w-full appearance-none rounded-lg border border-gray-300 dark:border-line-strong px-3 py-2 text-sm text-gray-700 dark:text-ink-dim uppercase focus:border-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-600/20 bg-white dark:bg-surface">
            ...
            <div>
              <label class="text-xs font-medium text-gray-500 dark:text-muted">Paid at</label>
              <input type="date" [ngModel]="paidAt()" (ngModelChange)="paidAt.set($event)"
                class="mt-1 w-full rounded-lg border border-gray-300 dark:border-line-strong px-3 py-2 text-sm text-gray-700 dark:text-ink-dim focus:border-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-600/20" />
            </div>
            <div>
              <label class="text-xs font-medium text-gray-500 dark:text-muted">Method</label>
...
export class SupplierPaymentModalComponent {
  ...
  readonly orderSupplierId = input.required<string>();
  ...
  openModal(): void {
    ...
    this.paidAt.set(this.todayLocal());
    this.method.set('');
    this.note.set('');
    this.open.set(true);
  }
  ...
  async submit(): Promise<void> {
    ...
    const legId = this.orderSupplierId();
    ...
    try {
      const paidIso = this.paidAt() ? new Date(`${this.paidAt()}T12:00:00`).toISOString() : undefined;
      await firstValueFrom(
        this.http.post<ApiResponse<SupplierPaymentDto>>(
          `${API_URL}/orders/${orderId}/suppliers/${legId}/payments`,
          {
            amount: amountStr,

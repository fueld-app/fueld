  ...
  template: `
    ...
    >
      ...
      <div class="flex h-16 items-center gap-3 px-3" [class.justify-center]="sidebarCollapsed()" [class.px-6]="!sidebarCollapsed()">
        <div class="brand-glyph">F</div>
        @if (!sidebarCollapsed()) {
          <span class="text-lg font-bold tracking-tight text-ink" style="font-family: var(--font-display);">Fueld</span>
        }
      </div>
...
export class MainLayoutComponent implements OnInit, OnDestroy {
  ...
  readonly navItems = computed(() => {
    ...
    return NAVIGATION.filter((item) => {
      ...
      // Filter out children that require features that are disabled
      if (item.children) {

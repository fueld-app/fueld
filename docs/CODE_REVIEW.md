...
- **`2e96f500`** — `auth.guard.ts` IP-allowlist now fails closed (403) on lookup errors + IPv6 CIDR matching (BigInt/128-bit); `backup.service.ts` redacts the DB password from pg_dump/psql stderr; `microsoft-oauth.service.ts` `getStateSigningKey` throws in production without a key + `validateReturnUrl` restricted to the APP_URL origin (open-redirect fix); `company.service.ts` segment `catKey` now bound as a parameter (no `sql.raw`).
- **`85aac73b`** — `geoip.ts` `lookupIp` rejects non-IP normalized values before the ip-api request (path/control-char injection via X-Forwarded-For); fixed the `geoip.test.ts` `uniqueIp` bug (signed `>> 8` produced negative octets / invalid IPs that the mock fetch silently accepted); added radix `10` to `parseInt(query.page/limit)` across credit/orders/companies/lloyds controllers.
